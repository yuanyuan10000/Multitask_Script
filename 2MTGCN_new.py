...
from smiles2graph import read_dataset
...
from ModelMTGCN import MTGCNPredictor,MTEGCNPredictor
...
def data_load(tasks_num, listname, dim_self_feat):
    ...
    for i in range(tasks_num):
        ...
        dataset, n_feats, smiles = read_dataset(listname[i])
...
save_path = "I:/4Multitask/exp/result_gcn/"
train_path = 'I:/4Multitask/exp/D_train'
test_path =  'I:/4Multitask/exp/D_test'
model_name = 'GCN'

epoch = 500

hidden_feats_shared = [100, 50]
hidden_feats_tower = [20]
...
# n_feats: 节点特征size
lengths = [n_feats_train, n_feats_train]
...
if model_name =='GCN':
    model = MTGCNPredictor(in_feats=n_feats, tasks_num=tasks_num,
                           hidden_feats_shared=hidden_feats_shared,
                           hidden_feats_tower=hidden_feats_tower,  # 两层GCN
                           n_tasks=n_tasks,  # 输出层神经元个数为2
...
for i in range(tasks_num):
    ...
    train_out.to_csv(save_path+'train_{}_{}_out.csv'.format(tasks_name[i],model_name), header=True, index=False)
...
for i in range(tasks_num):
    ...
    # train_out.to_excel(save_path+'train_{}_MTLnet_out.xlsx'.format(tasks_name[i]), header=True, index=False)
    test_out.to_csv(save_path+'test_{}_{}_out.csv'.format(tasks_name[i],model_name), header=True, index=False)
